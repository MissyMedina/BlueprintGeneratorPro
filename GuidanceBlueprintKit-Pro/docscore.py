...
def score(md):
    ...
    sec_points = 0
    for sec in [
        r"## 0\) Scope & Context", r"## 1\) Reality Check", r"## 2\) Upgrade PRD",
        r"## 3\) Operator Guide", r"## 4\) \(Optional\) Identity", r"## 5\) Appendices"
    ]:
        if has(sec, md): sec_points += 5
    total += sec_points; parts["sections"] = sec_points
    ...
    total += claim_points; parts["claims"] = claim_points
    ...
    total += rqm_points; parts["requirements"] = rqm_points

    # checklists
    check_points = 0
    if has(r"Quick Checklists", md): check_points += 5
    total += check_points; parts["checklists"] = check_points

    # word count factor
    words = len(md.split())
    wc_points = min(15, max(0, (words-300)//200))
    total += wc_points; parts["length"] = wc_points
