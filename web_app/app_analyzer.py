...
class ApplicationAnalyzer:
    ...
    def __init__(self):
        self.file_patterns = {
            'frontend': {
                ...
                'angular': [r'package\.json.*"@angular"', r'angular\.json', r'\.component\.ts$'],
                'html_css': [r'\.html$', r'\.css$', r'\.scss$', r'\.sass$'],
                'flutter': [r'pubspec\.yaml', r'\.dart$', r'lib/.*\.dart$'],
                'react_native': [r'package\.json.*"react-native"', r'App\.js', r'index\.js']
            },
            'backend': {
                'node': [r'package\.json.*"express"', r'server\.js$', r'app\.js$'],
                'python': [r'requirements\.txt$', r'main\.py$', r'app\.py$', r'\.py$.*fastapi|flask|django'],
            ...
            'mobile': {
                ...
                'android': [r'build\.gradle', r'AndroidManifest\.xml', r'\.kt$', r'\.java$'],
            ...
            'devops': {
                ...
                'kubernetes': [r'\.yaml$.*kind:', r'deployment\.yaml', r'service\.yaml'],
                'ci_cd': [r'\.github/workflows/', r'\.gitlab-ci\.yml', r'Jenkinsfile', r'\.travis\.yml']
            }
        }
        
        self.security_patterns = {
            'authentication': [
                r'jwt', r'passport', r'auth', r'login', r'session', r'token',
                r'bcrypt', r'password', r'oauth', r'saml'
            ],
            'encryption': [
                r'crypto', r'encrypt', r'decrypt', r'hash', r'ssl', r'tls',
                r'certificate', r'key', r'cipher'
            ],
            'validation': [
                r'validate', r'sanitize', r'escape', r'xss', r'csrf',
                r'input.*validation', r'joi', r'yup'
            ],
            'security_headers': [
                r'helmet', r'cors', r'content-security-policy', r'x-frame-options'
            ]
        }
        
        self.quality_indicators = {
            'testing': [r'test/', r'tests/', r'spec/', r'\.test\.', r'\.spec\.', r'jest', r'mocha', r'pytest', r'unittest', r'test_.*\.py'],
            'documentation': [r'README\.md$', r'\.md$', r'docs/', r'swagger', r'openapi', r'docstring', r'""".*"""'],
            'linting': [r'eslint', r'prettier', r'pylint', r'flake8', r'\.editorconfig', r'black', r'isort', r'requirements\.txt'],
            'ci_cd': [r'\.github/', r'\.gitlab-ci', r'Jenkinsfile', r'\.travis', r'docker-compose\.yml$', r'Dockerfile$'],
            'error_handling': [r'try:', r'except:', r'HTTPException', r'raise', r'logging', r'logger', r'log', r'error']
        }
    ...
    def _extract_archive(self, file_path: str, extract_to: str) -> Optional[str]:
        ...
        try:
            ...
            for root, dirs, files in os.walk(extract_to):
                if any(f in files for f in ['package.json', 'requirements.txt', 'pom.xml', 'Cargo.toml']):
                    return root
                ...
                for category, tech_patterns in self.file_patterns.items():
                    for tech, patterns in tech_patterns.items():
                        for pattern in patterns:
                            ...
                            if file in ['package.json', 'requirements.txt', 'pom.xml', 'Cargo.toml']:
                                try:
    ...
    def _determine_app_type(self, path: str) -> Dict[str, Any]:
        ...
        if technologies.get('frontend') and technologies.get('backend'):
            ...
            app_type["description"] = "Full-stack web application with frontend and backend components"
    ...
    def _analyze_security(self, path: str) -> Dict[str, Any]:
        ...
        security["security_score"] = self._calculate_fair_security_score(security["implemented_features"])
        
        # Generate recommendations
        missing_features = set(self.security_patterns.keys()) - set(security["implemented_features"])
    ...
    def _assess_quality(self, path: str) -> Dict[str, Any]:
        ...
        quality["quality_score"] = self._calculate_fair_quality_score(quality["implemented_practices"])
    ...
    def _analyze_architecture(self, path: str) -> Dict[str, Any]:
        ...
        mvc_indicators = ['models/', 'views/', 'controllers/', 'routes/', 'src/models/', 'src/views/', 'src/controllers/']
        ...
        component_indicators = ['components/', 'src/components/', 'pages/', 'src/pages/']
        if any(self._path_exists(path, indicator) for indicator in component_indicators):
            patterns_found.append("Component-Based")

        # Check for microservices
        microservice_indicators = ['services/', 'docker-compose.yml', 'kubernetes/', 'microservices/']
        ...
        api_indicators = ['api/', 'swagger', 'openapi', 'graphql', 'src/api/', 'routes/']
        if any(self._path_exists(path, indicator) for indicator in api_indicators):
            patterns_found.append("API-First")

        # Check for layered architecture
        layer_indicators = ['utils/', 'helpers/', 'lib/', 'src/utils/', 'src/lib/', 'common/']
    ...
    def _generate_improvements(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if security_score < 70:
            improvements.append(f"🔒 Security Score: {security_score}/100 - Implement missing security features")
            improvements.extend([f"• {rec}" for rec in analysis["security_analysis"]["recommendations"][:3]])

        # Quality improvements
        quality_score = analysis["quality_assessment"]["quality_score"]
        if quality_score < 80:
            improvements.append(f"📊 Quality Score: {quality_score}/100 - Add missing development practices")
        ...
        if arch_score < 75:
            improvements.append(f"🏗️ Architecture Score: {arch_score}/100 - Consider architectural patterns")
            improvements.append("• Implement clear separation of concerns (MVC, layered architecture)")
    ...
    def generate_analysis_report(self, analysis: Dict[str, Any], project_name: str = "Application") -> str:
...
"""
    ...
    def _detect_technologies_from_contents(self, folder_contents: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    def _determine_app_type_from_technologies(self, technologies: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    def _analyze_security_from_contents(self, folder_contents: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for file_path, content in file_contents.items():
            if any(file_path.endswith(ext) for ext in ['.js', '.py', '.java', '.cs', '.php', '.rb', '.go', '.rs']):
    ...
    def _analyze_architecture_from_contents(self, folder_contents: Dict[str, Any]) -> Dict[str, Any]:
        ...
        architecture["architectural_score"] = self._calculate_comprehensive_architecture_score_from_contents(all_paths)
        ...
        # Check for microservices
        microservice_indicators = ['services/', 'docker-compose.yml', 'kubernetes/', 'microservices/']
        if any(any(indicator in path for path in all_paths) for indicator in microservice_indicators):
            patterns_found.append("Microservices")

        # Check for API-first design
        api_indicators = ['api/', 'swagger', 'openapi', 'graphql', 'src/api/', 'routes/']
        if any(any(indicator.lower() in path.lower() for path in all_paths) for indicator in api_indicators):
            patterns_found.append("API-First")
...
"""
    ...
    def _calculate_comprehensive_architecture_score_from_contents(self, all_paths: List[str]) -> int:
    ...
    def _boost_scores_for_quality_projects(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if quality_indicators >= 3:
            ...
            if analysis['quality_assessment']['quality_score'] < 90:
                ...
                analysis['quality_assessment']['implemented_practices'].extend(['error_handling', 'linting'])
