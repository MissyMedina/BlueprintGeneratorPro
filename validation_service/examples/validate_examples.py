...
from client_sdk import ValidationClient, validate_project, validate_git_repo
import json
...
def example_batch_validation():
    ...
    for result in results:
        if result['status'] == 'success':
            ...
            print(f"{result['name']}: {avg:.1f}/100 (S:{scores['security']} Q:{scores['quality']} A:{scores['architecture']})")
        else:
